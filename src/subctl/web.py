...
def create_app(
    *,
    ...
    def user_subscription(request: Request, token: str, format_name: str) -> FileResponse:
...
def _fetch_source(request: Request) -> str:
    # The subctl-* prefix is reserved for checks generated by this project.
    # It survives the Caddy reverse proxy, while custom diagnostic headers may not.
    if request.headers.get("user-agent", "").lower().startswith("subctl-"):
        return "internal"
    return "client"
